...
#define dHIC(a,b,w,e) 
(crealf(sf_cmul(sf_cmul(conjf(a),b),w))/(sf_crealf(sf_cmul(conjf(a),a)) + e))
#endif
...
#define dCIC(a,b,e) (crealf(sf_cmul(conjf(a),b))/(crealf(sf_cmul(conjf(a),a)) + 
e))
#endif
...
/*< input data dimensions >*/ 
{
    ...
    sf_copyaxis(cub->af, af);  sf_setlabel(cub->af, "f" ); 
sf_setunit(cub->af,"Hz");
...
/*< initialize FIC mig >*/
{
    ...
    weop->swfl = sf_complexalloc3(sf_n(cub->amx),sf_n(cub->amy),cub->ompnth);
    weop->rwfl = sf_complexalloc3(sf_n(cub->amx),sf_n(cub->amy),cub->ompnth);
    weop->icic = sf_floatalloc4  
(sf_n(cub->amx),sf_n(cub->amy),sf_n(cub->az),sf_n(cub->aw));
    return weop;
}
...
/*< initialize HIC mig >*/
{
    ...
    weop->icic = sf_floatalloc3  (sf_n(cub->amx),sf_n(cub->amy),sf_n(cub->az));
    weop->ihic = sf_floatalloc4  
(sf_n(cub->ahx),sf_n(cub->ahy),sf_n(cub->aht),sf_n(cub->ac));
    return weop;
}
...
/*< initialize EIC mig >*/
{
    ...
    weop->ieic = sf_floatalloc5  
(sf_n(cub->ahx),sf_n(cub->ahy),sf_n(cub->ahz),sf_n(cub->aht),sf_n(cub->ac));
    return weop;
}
...
free(****weop->ieic);free(***weop->ieic);free(**weop->ieic);free(*weop->ieic);
free(weop->ieic);
}
...
#pragma omp parallel for schedule(dynamic,1)	\
	...
	/* frequency */
	w = sf_cmplx( cub->eps*sf_d(cub->aw), weisign * 
(sf_o(cub->aw)+iw*sf_d(cub->aw)) );
...
sf_seek(Fwfl,sizeof(sf_complex)*cub->nxy*(sf_n(cub->az)*iw+iz),SEEK_SET);
            ...
            for (iz=sf_n(cub->az)-1; iz>0; iz--) {
                weissr1(w,weop->wfld[ompith],cub,ssr,slo,iz,ompith,down);   /* 
wei   */
                weitap(   weop->wfld[ompith],tap);                          /* 
taper */
#ifdef _OPENMP
...
/*< wei wavefield >*/
{
    ...
    sou = 
sf_complexalloc4(sf_n(cub->amx),sf_n(cub->amy),sf_n(cub->az),cub->ompnth);
    weisign=causal?+1:-1;
...
#else
	    YXLOOP( weop->wfld[ompith][iy][ix] = 
sf_cadd(weop->wfld[ompith][iy][ix],sou[ompith][iz][iy][ix]); );
#endif
#ifdef _OPENMP
#pragma omp critical
#endif
...
#else
		YXLOOP( weop->wfld[ompith][iy][ix] = 
sf_cadd(weop->wfld[ompith][iy][ix],sou[ompith][iz+1][iy][ix]); );
#endif
#ifdef _OPENMP
#pragma omp critical
#endif
...
#pragma omp parallel for schedule(dynamic,1)	\
	...
	/* frequency */
	ws = sf_cmplx( cub->eps*sf_d(cub->aw), (+1) * 
(sf_o(cub->aw)+iw*sf_d(cub->aw)) );
	wr = sf_cmplx( cub->eps*sf_d(cub->aw), (-1) * 
(sf_o(cub->aw)+iw*sf_d(cub->aw)) );
...
/*< CIC migration >*/
{
    ...
    /*------------------------------------------------------------*/
    if(cub->verb) fprintf(stderr,"zero CIC image...");
    if(cub->verb) fprintf(stderr,"Starting from iw index: %i ... to %i \n", start,end);
...
#endif
	{
	    if(cub->verb) sf_warning ("WEI Working on ... (ith=%2d) ... <iw=%3d of %3d>",
				      ompith,iw+1,sf_n(cub->aw));
...
#else
            YXLOOP( weop->swfl[ompith][iy][ix] = 
sf_cadd(weop->swfl[ompith][iy][ix],img[iz][iy][ix]); );
#endif
            weitap(   weop->swfl[ompith],tap);
            weissr(ws,weop->swfl[ompith],cub,ssr,slo,iz-1,ompith);
        } /* z */
...
#else
        YXLOOP( weop->swfl[ompith][iy][ix] = 
sf_cadd(weop->swfl[ompith][iy][ix],img[0][iy][ix]); );
#endif
        weitap(   weop->swfl[ompith],tap);
#ifdef _OPENMP
#pragma omp critical
...
/*< apply HIC at one z >*/
{
	...
	for(jc=0;jc<eico->ncofz[iz];jc++) {
	    ...
	    for        (iht=0; iht<sf_n(cub->aht); iht++) { wt 
=eico->tt[iw][iht];
		for    (ihy=0; ihy<sf_n(cub->ahy); ihy++) { 
mcy=eico->mcyall[ic][ihy]; pcy=eico->pcyall[ic][ihy];
		    for(ihx=0; ihx<sf_n(cub->ahx); ihx++) { 
mcx=eico->mcxall[ic][ihx]; pcx=eico->pcxall[ic][ihx];
...
/*< apply forward C.I.C. >*/
{
	...
	for(  iy=0; iy<sf_n(cub->amy); iy++){
		...
		if(conj)
		    cic[iz][iy][ix] += 
cWGH(swfl[iz][iy][ix],rwfl[iz][iy][ix],sf_cmplx(0.0,-0.69));
		else
		    cic[iz][iy][ix] += 
cAWGH(swfl[iz][iy][ix],rwfl[iz][iy][ix],sf_cmplx(0.0,-0.69));
#else
		if(conj)
		    cic[iz][iy][ix] = 
sf_cadd(cic[iz][iy][ix],cWGH(swfl[iz][iy][ix],rwfl[iz][iy][ix],sf_cmplx(0.0,-0.
69)));
		else
		    cic[iz][iy][ix] = 
sf_cadd(cic[iz][iy][ix],cAWGH(swfl[iz][iy][ix],rwfl[iz][iy][ix],sf_cmplx(0.0,-0.
69)));
#endif
	    } /* loop over ax */
	} /* loop over ay */
...
=eico->tt[iw][iht];
		    for        (ihz=0; ihz<sf_n(cub->ahz); ihz++) { 
mcz=eico->mczall[ic][ihz]; pcz=eico->pczall[ic][ihz];
			for    (ihy=0; ihy<sf_n(cub->ahy); ihy++) { 
mcy=eico->mcyall[ic][ihy]; pcy=eico->pcyall[ic][ihy];
			    for(ihx=0; ihx<sf_n(cub->ahx); ihx++) { 
mcx=eico->mcxall[ic][ihx]; pcx=eico->pcxall[ic][ihx];
