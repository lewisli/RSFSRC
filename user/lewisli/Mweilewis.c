...
int main (int argc, char *argv[])
{
    ...
    if(!sf_getint("cigsplit", &cigsplit)) cigsplit = 1;
    if(!sf_getint("splitindex", &splitindex)) splitindex = 0;
  ...
  /*------------------------------------------------------------*/
	...
	case 'c': /* CIC */
	    ...
	    if(verb) sf_warning("Preparing Output File");
	    Fcic = sf_output("out"); sf_settype(Fcic,SF_FLOAT);
	    weicic_out(cub,Fcic); /* prepare output image */
	    break;
	case 'f': /* CIC */
    ...
    int totalZ = sf_n(cub->aw);
    int stepSize = totalZ/cigsplit;

    startIndex = splitindex*stepSize;
    endIndex = (splitindex+1)*stepSize-1;

    fprintf(stderr,"StartIndex: %d, EndIndex: %d, TotalZ: %d\n", startIndex, endIndex, totalZ);
	...
	case 'c': /* conventional IC */
	    ...
	    weicic(weop,cub,ssr,tap,slo,Fsou,Frec,Fcic,startIndex, endIndex);
	    weicic_close(weop);
