/* 2-D SC-seislet: Seislet transform with differential shot continuation
...
{
    int ns, nh, iw, nw, nsh, i4, n4;
    float s0, ds, h0, dh, w0, dw, w, eps; 
    ...
    pp = sf_complexalloc(nsh);
    qq = sf_complexalloc(nsh);
    ...
    sclet_init(nh,dh,h0,ns,ds,dw,s0,inv,unit,eps,type[0]);

    for (i4=0; i4 < n4; i4++) {
	for (iw=0; iw < nw; iw++) {
	    if (verb) sf_warning("frequency %d of %d",iw+1,nw);
	    sf_complexread(pp,nsh,in);
	    w = w0+iw*dw;
	    if (adj) {
		sclet_lop(adj,false,nsh,nsh,qq,pp,w);
	    } else {
		sclet_lop(adj,false,nsh,nsh,pp,qq,w);
	    }
	    sf_complexwrite(qq,nsh,out);
	}
    }

    exit(0);
}
/* 	$Id: Msclet.c 5035 2009-12-01 01:29:11Z yang_liu $	 */
