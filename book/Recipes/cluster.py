...
----------------------------------------------------------------
Adapted for CEES Cluster & Stanford University

author: Lewis Li, Oct 9th 2014, SCRF
'''
...
#    lines.append('echo "JOB: %s running" >> pbs/jobs.txt' % name)
    ...
    if parallel:
        ...
        if relaunch or run:
          ...
          lines.append('RunSCons -f %s/SConstruct-%s' % (pbs_dirt,name))
        #lines.append(tasks+'\n')
...
''' % (sconsign))
		    ...
		    sconspath = '/data/cees/lewisli/scons/scons-2.3.4/bin/scons'
                    ...
                    file.write(string.join(['%s %s -f %s/SConstruct-%s'% (python27path,sconspath,pbs_dirt,name) for name in names],'\n'))
                    file.close()
                    ijob += 1
